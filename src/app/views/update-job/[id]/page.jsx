...
import { addJob, fetchJobById, updateJob } from "@/app/store/slices/jobSlice";
import { FiLock } from "react-icons/fi";
import { Status } from "@/app/globals/status";
import { useRouter } from "next/navigation";
import { useDispatch, useSelector } from "react-redux";

export default function UpdateJob({ params }) {
  ...
  useEffect(() => {
    if (singleJob) {
      setCompanyLogoUrl(singleJob?.companyLogo);
      setFormData({
        ...
        experience: singleJob?.experience || "Entry-Level",
        jobType: singleJob?.jobType || "Full-Time",
        jobMode: singleJob?.jobMode || "onsite",
        deadline: new Date(singleJob?.deadline) || new Date(),
        description: singleJob?.description || "",
        requirements: singleJob?.requirements || "",
        createdBy: singleJob?.createdBy || { email: "", id: "", name: "" },
      });
    }
  }, [singleJob]);
        ...
        <div className="w-full max-w-4xl border border-[#FF5722] p-4 sm:p-8 rounded-xl shadow-lg">
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                ...
                >
                  {({ open }) => (
                    ...
                    >
                      {companyLogoName ? companyLogoName : companyLogoUrl}
                    </button>
                  )}
                </CldUploadWidget>
              </div>
            ...
            <div className="text-center mt-4">
              <button
                type="submit"
                className="w-full bg-[#FF5722] text-white py-2 rounded-lg font-semibold"
              >
                Update Job
              </button>
            </div>
